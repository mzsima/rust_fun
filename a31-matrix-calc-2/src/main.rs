...
fn setup(model: &Model) {
  let mtx_a: Mat2d = vec![vec![1, 0], vec![0, 1], vec![0, 1], vec![1, 0]];
  let mtx_b: Mat2d = vec![vec![1, 1], vec![0, 1]];
  let mtx_c: Mat2d = vec![vec![1, 0, 0, 1], vec![0, 1, 1, 0]];
  let res = mult_mtx(&mult_mtx(&mtx_a, &tr_mtx(&mtx_b)), &mtx_c);
  println!("{:?}", res);

  draw_matrix(&model, &mtx_a, 0., 0., BLACK);
  draw_matrix(&model, &mtx_b, 0., 80., BLUE);
  draw_matrix(&model, &mtx_c, 40., 80., ORANGE);
  draw_matrix(&model, &res, 40., 0., GREEN);
}

fn draw_matrix(model: &Model, mtx: &Mat2d, x: f32, y: f32, c: Srgb<u8>) {
  for i in 0..mtx.len() {
    for j in 0..mtx[0].len() {
      let color = if mtx[i][j] == 0 { BLACK } else { WHITE };
      model
        .draw
        .rect()

...
fn setup(model: &Model) {
  let mtx_b = create_random_matrix(16);
  let mtx_a = create_matrix_a(180, 16);
  ...
  draw_matrix(&model, &res, 32., 32., SEAGREEN, SEASHELL);
}

fn create_random_matrix(size: usize) -> Mat2d {
  ...
  for i in 0..mtx.len() {
    for j in 0..mtx[0].len() {
      ...
      model
        .draw
        .rect()
        .xy(pt2(x + 2. * j as f32, y + 2. * i as f32))
        .w_h(2., 2.)
        .color(color)
        .stroke_weight(0.5)
        .stroke(GRAY);
